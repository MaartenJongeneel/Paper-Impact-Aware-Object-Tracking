function [AH_B, BV_AB, FN, FT] = BoxSimulator(releasePosition,releaseOrientation,releaseLinVel,releaseAngVel,eN,eT,mu,box,AR_C,Ao_C,dt,endtime)
...
a     = 0.01;              %Prox point auxilary parameter           [-]
...
endframe = round(endtime/dt);   %Run to this frame                       [-]
...
while run 
    ...
    %Determine if box is at rest pose
    if  ii == endframe
       run = false;
    end
    ii = ii+1;
