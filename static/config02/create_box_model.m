...
%% Cuboid model
% This script computes the model for the cuboid. It gives the following outputs:
% INPUTS:    none
%
...
l      = 0.100;  %Length of the cuboid: X-direction   [m]
w      = 0.150;  %Width  of the cuboid: Y-direction   [m]
h      = 0.050;  %Height of the cuboid: Z-direction   [m] 
...
offset = 0.010;  %Offset for computing normal to face [m]
doSave = false;  %Decide if you want to save results  

%% Define vertices for normal and bigger model and face centers
box.dim = [l,w,h];
vert  = [l -l -l l l -l -l l; -w -w w w -w -w w w; -h -h -h -h h h h h]/2;
vert_b = vert+sign(vert)*sd;
...
figure;
  ...
  for jj=1:12      
      ...
      two_vertices=[vert(:,edges(1,jj)),vert(:,edges(2,jj))];
